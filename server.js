...
function carpismaVarMi(x, y, yaricap) {
    for (let d of DUVARLAR) {
        ...
        if ((mesafeX * mesafeX) + (mesafeY * mesafeY) < (yaricap * yaricap)) return true;
    }
    return false;
}
...
// HTML Şablonu (Hem ana dizin hem oyun-alani buraya yönlendirildi)
const oyunHtmlSayfasi = `
<!DOCTYPE html>
<html lang="tr">
<head>
    <meta charset="UTF-8">
...
// İKİ ROTAYI DA TANIMLADIK: Artık / da /oyun-alani da patlamayacak!
app.get('/', (req, res) => { res.send(oyunHtmlSayfasi); });
app.get('/oyun-alani', (req, res) => { res.send(oyunHtmlSayfasi); });
